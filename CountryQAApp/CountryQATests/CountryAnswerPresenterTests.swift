...
final class CountryAnswerPresenterTests: XCTestCase {
    ...
    func test_presentError_displaysErrorMessageWithRetryBoundToTheQuestion() {
        let (sut, spy) = makeSUT()

        sut.presentError(retrying: "Capital of Belgium?")

        XCTAssertEqual(spy.messages, [
            CountryAnswerViewModel(
                message: "Something went wrong loading the answer. Please try again.",
                retryQuestion: "Capital of Belgium?"
            )
        ])
        XCTAssertEqual(spy.messages.first?.showsRetry, true)
    }
