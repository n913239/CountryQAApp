...
public final class ChatViewController: UIViewController, CountryAnswerView {
    ...
    /// One owned task per flag URL: the key stops a reloading row from starting a duplicate
    /// download, and holding the handles means the work cannot outlive the screen.
    private var flagImageLoads: [URL: Task<Void, Never>] = [:]

    deinit {
        flagImageLoads.values.forEach { $0.cancel() }
    }
...
extension ChatViewController: UITableViewDataSource {
        ...
        case let .bot(viewModel):
            cell.configure(
                ...
                onRetry: { [weak self] in
                    guard let question = viewModel.retryQuestion else { return }
                    self?.onAsk?(question)
                }
            )
    ...
    private func loadFlagImage(from url: URL, for row: Int) {
        guard flagImages[url] == nil, flagImageLoads[url] == nil else { return }

        flagImageLoads[url] = Task { [weak self] in
            guard let data = await self?.imageLoader?(url),
                  let image = UIImage(data: data),
                  !Task.isCancelled else {
                self?.flagImageLoads[url] = nil
                return
            }

            guard let self, row < bubbles.count else { return }
            flagImages[url] = image
            flagImageLoads[url] = nil
